...
if archivo:
    try:
        ...
        # 🧭 TABS
        # ------------------------------------------------------------------
        tab1, tab2, tab3 = st.tabs(["🌐 Global", "👥 Clientes", "📦 Exportar"])
        ...
        with tab1:
            ...
            fig = px.bar(fc_df.head(25), x="CLIENTE", y="TOTAL", title="Top clientes")
        ...
        with tab2:
            ...
            with col1:
                top_prod = resultado.sort_values("Facturacion", ascending=False).head(20)
            ...
            with col2:
                mat = resultado.groupby("MATERIAL")["Facturacion"].sum().reset_index()
                mat = mat.sort_values(by="Facturacion", ascending=False)
